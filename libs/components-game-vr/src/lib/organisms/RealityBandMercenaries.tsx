...
const RealityBandMercenaries = () => {
  ...
  const [inspectMercenaryId, setInspectMercenaryId] = useState<number>(0);
  const refSelectMercenaryBox = useRef<Mesh>();
  const refInspectMercenaryBox = useRef<Mesh>();
  const selectPosition = useMemo(() => new Vector3(0.23, 0.18, 0), []);
  const inspectPosition = useMemo(() => new Vector3(0.23, -0.05, 0), []);
  const bandMercenaries = useListMercenariesById(band?.mercenaryIds);

  return <>
    <group position={[0, .15, 0]}>
      {bandMercenaries?.map((mercenary, index) => <RealityBandMercenary key={mercenary.mercenaryId} mercenary={mercenary} offset={index} setInspectMercenaryId={setInspectMercenaryId} refSelectMercenaryBox={refSelectMercenaryBox} refInspectMercenaryBox={refInspectMercenaryBox} />)}
    </group>
    <RealityBox transparent opacity={0.5} color='orange' ref={refSelectMercenaryBox} position={selectPosition} args={[.25, .15, .05]}>
      ...
      <RealityText text='(Grab &amp; drop here)' position={[0, -.09, 0.03]} fontSize={0.025} />
    </RealityBox>
