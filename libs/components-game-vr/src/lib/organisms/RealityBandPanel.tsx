...
const RealityBandPanel: React.FC = () => {
  ...
  return <group>
    ...
    <Billboard>
      ...
      <RealityText position={[0, 2.05, 0]} text={`${band?.name.substring(0, 8)}'s Mercenary Band`} fontSize={0.75} />
      <RealityText position={[-2, 1.25, 0]} text={`${band?.gold} Gold`} />
      <RealityText position={[2, 1.25, 0]} text={`${band?.actionPoints} AP`} />
      <RealityText position={[0, .75, 0]} text={`${band?.mercenaryIds.length}/${getMaxBandMercenaries(band)} Mercenaries`} />
