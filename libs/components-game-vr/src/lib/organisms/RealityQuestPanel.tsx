...
const RealityQuestPanel: React.FC<Props> = ({refSelectBox}) => {
  const selectBoxPosition = useMemo(() => new Vector3(0, -0.4, .05), []);
  const goQuestPosition = useMemo(() => new Vector3(0, -0.34, .01), []);
  const boardPosition = useMemo(() => new Vector3(0, .1, -.002), []);
  const [questResults, setQuestResults] = useState<T_QuestResults>();
  const {questRunnerText} = useQuestRunner();

  return <>
    <group position={[0, 0, -1]}>
      <RealityBox color="#533" transparent opacity={0.9} args={[1.5, 1, 0.001]} position={boardPosition} />
      {questResults && <>
      ...
      <RealityText position={[0, -.25, 0]} fontSize={0.05} color="#CCC" text={questRunnerText} />
