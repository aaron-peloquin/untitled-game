...
const QuestItem: React.FC<Props> = ({quest}) => {
  ...
  return <Card layer="4" heading={`${quest.type} ${quest.targetName}`}>
    <GridTemplate gridTemplateAreas={QUEST_AREAS} justifyItems="center" textAlign='center' columns={2}>
      <GridArea name='difficulty'><Output id={`quest_${quest.questId}_difficulty`} value={quest.level} label="Difficulty" /></GridArea>
      <GridArea name="target____"><Output id={`quest_${quest.questId}_target`} value={`${stats.ethnicity} ${stats.profession}`} label="Target" /></GridArea>
